...
log: ## Show symfony logs.
	$(SYMFONY) server:log
.PHONY: log

dc: ## Create symfony database.
	$(SYMFONY_CONSOLE) doctrine:database:create --if-not-exists
.PHONY: dc

dd: ## Drop symfony database.
	$(SYMFONY_CONSOLE) doctrine:database:drop --if-exists --force
.PHONY: dd

su: ## Update symfony schema database.
...
sudo-perm: ## Fix permissions with sudo.
...
dump-env-container: ## Dump Env container.
	$(SYMFONY_CONSOLE) debug:container --env-vars
.PHONY: dump-env-container
...
first-install: docker-up composer-install perm dc dmm open ## First install.
.PHONY: first-install

start: docker-up open ## Start project.
.PHONY: start

stop: docker-stop stop ## Stop project.
.PHONY: stop
